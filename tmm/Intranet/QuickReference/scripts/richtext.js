...
function writeRTE(rte, html, width, height, buttons, readOnly) {
	...
	if (buttons == true) {
		...
		document.writeln('	<tr>');
		document.writeln('		<td><img id="bold" class="rteImage" src="' + imagesPath + 'bold.gif" width="25" height="24" alt="Bold" title="Bold" onClick="FormatText(\'' + rte + '\', \'bold\', \'\')"></a></td>');
		...
		document.writeln('		<td><img class="rteVertSep" src="' + imagesPath + 'blackdot.gif" width="1" height="20" border="0" alt=""></td>');
		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'hyperlink.gif" width="25" height="24" alt="Insert Link" title="Insert Link" onClick="initGlobals(\'' + rte + '\'); openWindow(\'FileList.aspx\', \'LinkList\', 400, 500);"></td>');
...
//		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'redo.gif" width="25" height="24" alt="Redo" title="Redo" onClick="FormatText(\'' + rte + '\', \'redo\')"></td>');
	...
	document.writeln('<iframe id="' + rte + '" name="' + rte + '" width="' + width + 'px" height="' + height + 'px" src="blank.html"></iframe>');

/*
Do not allow the user to view the source	
	if (!readOnly) document.writeln('<br /><input type="checkbox" id="chkSrc' + rte + '" onclick="toggleHTMLSrc(\'' + rte + '\');" />&nbsp;View Source');
*/

	document.writeln('<iframe width="154" height="104" id="cp' + rte + '" src="palette.htm" marginwidth="0" marginheight="0" scrolling="no" style="visibility:hidden; display: none; position: absolute;"></iframe>');
...
function initGlobals(rte)
{
	// set the globals, these are copies of this functions objects, they are used by the popup window.
//	oRET2 = oRTE;
	rte2 = rte;
}

function AddLink(strFileLink, command)
{
	var oRTE;
	if (document.all) {
		oRTE = frames[rte2];
		...
		oRTE = document.getElementById(rte2).contentWindow;
	...
	if (command == "createlink") {
		...
		var szURL = strFileLink;
		try {
			//ignore error for blank urls
			oRTE.document.execCommand("Unlink", false, null);
			oRTE.document.execCommand("CreateLink", false, szURL);	
					
			//set the target of all links to their own window.
			for(i=0; i<oRTE.document.links.length; i++)
			{
				oRTE.document.links[i].target = "blank";
			}
...
function AddImage(rte) {
	...
	// popup a window that lists the files currently on the webserver
	// the popup window will set the imagePath variable.
	openWindow("FileList.aspx?Images=1", "ImageList", 400, 500);
...
// function is exactly the same as the addimage() only it can be called from a popup window.
// this function uses variables that are global to the class, therefore the popup can alter them.
function insertImage()
{
	if (document.all) {
		...
		//get current selected range
		var selection = oRTE2.document.selection; 
		if (selection != null) {
			rng = selection.createRange();
		}
	} else {
		oRTE2 = document.getElementById(rte2).contentWindow;
		
		//get currently selected range
		var selection = oRTE2.getSelection();
		rng = selection.getRangeAt(selection.rangeCount - 1).cloneRange();
	}

	if ((imagePath != null) && (imagePath != "")) {
		oRTE2.focus();
		oRTE2.document.execCommand('InsertImage', false, imagePath);
		oRTE2.focus();
	}
}
...
{
	var MyWin = window.open(url, winName, "height=" + height + ",width=" + width + ",toolbar=no");
	MyWin.focus();
}
